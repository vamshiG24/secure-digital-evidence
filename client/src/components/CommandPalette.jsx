...
import { Search, FolderOpen, Shield, User, Sparkles, ClipboardList, PlusCircle, Zap, X, CornerDownLeft, Activity } from 'lucide-react';
      ...
      onClick={onClose}>
        <motion.div
          ...
          style={{
            ...
            borderRadius: 20, boxShadow: '0 25px 60px -12px var(--overlay)',
          ...
          <div style={{
            ...
            {query && (
              <button onClick={() => setQuery('')}
                style={{ border: 'none', background: 'transparent', cursor: 'pointer', color: 'var(--text-faint)' }}>
                <X size={16} />
              </button>
            )}
            <kbd style={{
              background: 'var(--border-strong)', color: 'var(--text-secondary)', fontSize: 11, fontWeight: 700,
              padding: '2px 7px', borderRadius: 6, border: '1px solid var(--border-strong)'
            }}>
              ESC
            </kbd>
          </div>
          ...
          <div style={{ maxHeight: 420, overflowY: 'auto', padding: '12px 14px' }}>
            {loading && (
              <div style={{ padding: '16px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <Zap size={14} className="animate-spin" color="var(--primary)" /> Scanning digital forensic vault...
              </div>
            )}
            ...
            {results.cases.length > 0 && (
              <div style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', color: 'var(--text-faint)', letterSpacing: '0.06em', padding: '6px 10px' }}>
                ...
                {results.cases.map((c) => (
                  <div key={c._id}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--primary-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#4338ca' }}>
                        <FolderOpen size={16} />
                      </div>
                      <div>
                        <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text-primary)' }}>{c.title}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          <span style={{ fontFamily: 'monospace', fontWeight: 600, color: 'var(--primary)' }}>{c.caseNumber || 'CASE'}</span> • Priority: {c.priority} • Status: {c.status}
                        </div>
                      </div>
                    </div>
                    <CornerDownLeft size={13} color="var(--text-faint)" />
                  </div>
                ))}
              </div>
            )}
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                {results.evidence.map((ev) => (
                  <div key={ev._id}
                    ...
                    onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--success-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--success)' }}>
                        <Shield size={16} />
                      </div>
                      <div style={{ maxWidth: 440 }}>
                        ...
                        <div style={{ fontSize: 10.5, fontFamily: 'monospace', color: 'var(--text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          SHA-256: {ev.fileHash}
                        </div>
                      </div>
                    </div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                {results.users.map((u) => (
                  <div key={u._id}
                    ...
                    onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--purple-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--purple)' }}>
                        <User size={16} />
                      </div>
                      <div>
                        <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text-primary)' }}>{u.name}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
              ...
              {filteredActions.map((act, idx) => {
                ...
                return (
                  <div key={idx}
                    ...
                    onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: 8, background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)' }}>
                        <Icon size={16} />
                      </div>
                    ...
                    <span style={{
                      ...
                      padding: '3px 8px', borderRadius: 6, background: 'var(--surface-2)', color: 'var(--text-muted)'
                    }}>
                      {act.badge}
                    </span>
                  </div>
          ...
          <div style={{
            ...
            background: 'var(--surface)', fontSize: 11, color: 'var(--text-muted)'
          }}>
            <span>Forensic Omnibar v2.0 • Merkle Ledger Active</span>
            <div style={{ display: 'flex', gap: 12 }}>
              <span>Press <kbd style={{ padding: '2px 5px', background: 'var(--surface)', borderRadius: 4, border: '1px solid var(--border-strong)' }}>↵</kbd> to select</span>
              <span><kbd style={{ padding: '2px 5px', background: 'var(--surface)', borderRadius: 4, border: '1px solid var(--border-strong)' }}>esc</kbd> to close</span>
            </div>
          </div>
